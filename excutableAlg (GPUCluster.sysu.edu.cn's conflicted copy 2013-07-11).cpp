...
void train(vector<string>& file)
{
	...
	else 
	{
		...
		else
		{
			...
			HogFeature fe(file, writer.c_str(), ef);
...
void laterProcess(Mat& data, string writer, bool train)
{
	...
	if(train)
	{
		int retainedComponent = data.cols;
		PCA pca(data, noArray(), CV_PCA_DATA_AS_ROW, retainedComponent);
...
void method_pca(Mat& orginalMat, Mat& pcaMat)
{
	...
	bool* anormal = decideAnormal(score, sum(score, n)/n, n);
